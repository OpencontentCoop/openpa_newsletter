...
<div class="newsletter newsletter-configure Prose u-padding-bottom-l">
    ...
    <form class="Form Form--spaced" role="form" name="configure" method="post" action={concat('/newsletter/configure/', $newsletter_user.hash)|ezurl}>
    ...
    {* for every newsletter_system check if there are newsletter_list where a subscription for current siteaccess is possible *}
        ...
        {else}
                ...
                <div class="newsletter-system-design  u-margin-top-xl u-margin-bottom-xl">
                ...
                <fieldset class="Form-field Form-field--choose Grid-cell">
                  <legend class="Form-legend">{"Salutation"|i18n( 'cjw_newsletter/configure' )}:</legend>
                  {foreach $available_saluation_array as $saluation_id => $salutataion_name}                                            
                    <label class="Form-label Form-label--block">
                      <input type="radio" class="Form-input" name="Subscription_Salutation" value="{$saluation_id}"{if $newsletter_user.salutation|eq($saluation_id)} checked="checked"{/if} title="{$salutataion_name|wash}" />
                      <span class="Form-fieldIcon" role="presentation"></span> {$salutataion_name|wash}
                  </label>
                  {/foreach}                  
                </fieldset>
                ...
                <div class="Form-field">
                  <label class="Form-label" for="Subscription_FirstName">{"First name"|i18n( 'cjw_newsletter/configure' )}:</label>
                  <input class="Form-input" id="Subscription_FirstName" type="text" name="Subscription_FirstName" value="{$newsletter_user.first_name|wash}" title="{'First name of the subscriber.'|i18n( 'cjw_newsletter/configure' )}"{*cond( is_set( $user ), 'disabled="disabled"', '')*} />
                </div>

                {* Last name. *}
                <div class="Form-field">
                  <label class="Form-label" for="Subscription_LastName">{"Last name"|i18n( 'cjw_newsletter/configure' )}:</label>
                  <input class="Form-input" id="Subscription_LastName" type="text" name="Subscription_LastName" value="{$newsletter_user.last_name|wash}"{*cond( is_set( $user ), 'disabled="disabled"', '')*} />
                </div>

                {* Email. *}
                <div class="Form-field">
                  <label class="Form-label" for="Subscription_Email">{"Email"|i18n( 'cjw_newsletter/configure' )}*:</label>
                  <input class="Form-input" id="Subscription_Email" type="text" name="Subscription_Email" value="{$newsletter_user.email|wash}" title="{'Email of the subscriber.'|i18n( 'cjw_newsletter/configure' )}" {cond( is_set( $newsletter_user ), 'disabled="disabled"', '')} />
                </div>
