...
{def $sitaccess_name = site_identifier()
     $suffix = 'frontend'}
{if $attribute.language_code|ne('ita-IT')}
    {set $sitaccess_name = concat($sitaccess_name, '_', $attribute.language_code|explode('-')[0])}
{/if}
{if ezini('ExtensionSettings', 'ActiveAccessExtensions')|contains('openpa_agenda')}
    {set $suffix = 'agenda'}
{/if}
{set $sitaccess_name = concat($sitaccess_name, '_', $suffix)}
{$sitaccess_name}
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_MainSiteaccess_{$attribute.id}" value="{$sitaccess_name}" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_EmailSender_{$attribute.id}" value="{$email_sender}" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_EmailSenderName_{$attribute.id}" value="{ezini('SiteSettings', 'SiteName')}" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_EmailReplyTo_{$attribute.id}" value="{$email_sender}" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_EmailReturnPath_{$attribute.id}" value="{$email_sender}" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_EmailReceiverTest_{$attribute.id}" value="{$email_receiver_test}" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_AutoApproveRegisterdUser_{$attribute.id}" value="0" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_OutputFormatArray_{$attribute.id}[]" value="0" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_OutputFormatArray_{$attribute.id}[]" value="1" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_SkinName_{$attribute.id}" value="bootstrapitalia" />
<input type="hidden" name="{$attribute_base}_CjwNewsletterList_PersonalizeContent_{$attribute.id}" value="0" />
